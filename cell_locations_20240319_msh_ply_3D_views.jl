    ...
    for iter in range(1,1000)
       # @show iter
        for (ic,cell) in enumerate(eachrow(cells))
            ...
            for j in range(1,npc)
                @inbounds phi_node[cell[j]] += phi[ic]*weight_node[ic,j]               
            end
        end
            ...
            # @show bface[5], bvalue.ID
                if bface[5] == bvalue.ID
                    ...
                    if bvalue.type == 1 # Dirichlet
                        # @show bvalue.value
                        @inbounds phi_node[bface[1]] = bvalue.value
                        @inbounds phi_node[bface[2]] = bvalue.value
                    end
                end
            end
        end
        ...
        for (ic,cell) in enumerate(eachrow(cells))
            sourceₛ[ic] = 0.0
            for j in range(1,npc)
                @inbounds sourceₛ[ic] += t̂_dot_lf[ic,j]*ℾ/δ_face[cell[j+npc]]*(phi_node[cell[circle_path[j,2]]] - phi_node[cell[circle_path[j,1]]])
        ...
        for (ic,cell) in enumerate(eachrow(cells))
           ...
           @inbounds phi[ic] = (sourceᵤ[ic] + sourceₛ[ic])/aₚ[ic]
            for j in range(1,npc)
                
                if cell[j+npc*2] > 0
                    @inbounds phi[ic] += aₙ[ic,j]*phi[cell[j+npc*2]]/aₚ[ic]
                    #@show iter,cell[j+npc*2],phi[cell[j+npc*2]],aₙ[ic,j],phi[ic],aₙ[ic,j]*phi[cell[j+npc*2]]
                end
                
                @inbounds phi_node[cell[j]] = 0.0 
            end
